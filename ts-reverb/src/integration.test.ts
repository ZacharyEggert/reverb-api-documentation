...
describe("Reverb Integration Tests", () => {
  let client: Reverb;
  let testListingId: number | undefined;
  let testListingUuid: string | undefined;

  beforeAll(() => {
    const token = process.env.REVERB_TOKEN;
    if (!token) {
      throw new Error(
        "REVERB_TOKEN environment variable is required for integration tests.\n" +
	...
	describe("Account", () => {
		test("should fetch account details", async () => {
			...
			const shopId = (account as any).shop?.id;
			...
			if (!shopId) {
				...
				console.warn(
					"   Most API endpoints require a shop. Create one at:"
				);
				console.warn(
					"   https://sandbox.reverb.com/my/shop/edit\n"
				);
			}
		});
	});
    ...
    test("should list drafts", async () => {
      ...
      console.log("✓ Draft listings count:", (result as any).total || 0);
    });
	...
	test("should create a draft listing", async () => {
		...
		const listing = await client.createListing({
			...
			testListingId = (listing as any).id || (listing as any).listing?.id;
			if (testListingId) {
				console.log("✓ Created draft listing ID:", testListingId);
			} else {
				console.log("⊘ Listing created but no ID in response");
				console.log("  Response keys:", Object.keys(listing as any));
			}
		}, 10000);
	...
	test("should update the draft listing", async () => {
		...
		try {
			...
			const listings = (drafts as any).listings || [];
			const found = listings.find((l: any) => l.title?.includes(testListingUuid!));
	...
	}, 10000);		test("should get listing images", async () => {
		if (!testListingUuid) {
			console.log("⊘ Skipped - no test UUID");
			return;
		}
		...
		console.log(
			"✓ Listing images:",
			Array.isArray(images) ? images.length : "unknown"
		);
	} catch (err) {
		if (err instanceof ReverbApiError && err.status === 404) {
			console.log("⊘ Listing no longer available (404)");
				...
				if (err instanceof ReverbApiError) {
					if (err.status === 404) {
						...
						try {
							...
							if (
								endErr instanceof ReverbApiError &&
								endErr.status === 404
							) {
								console.log("⊘ Listing already gone (404)");
							} else {
	...
	describe("Sales", () => {
		test("should list seller sales", async () => {
			try {
				...
				console.log(
					"✓ Seller sales count:",
					Array.isArray((sales as any).sales)
						? (sales as any).sales.length
						: 0
				);
			} catch (err) {
				if (
					err instanceof ReverbApiError &&
					err.status === 401 &&
  ...
  describe("Vacation Mode", () => {
    test("should get vacation status", async () => {
      ...
      console.log("✓ Vacation status:", (status as any).enabled || false);
    });
  });
      ...
      try {
		...
		test("should handle validation errors", async () => {
			try {
				...
				expect(error).toBeInstanceOf(ReverbApiError);
				console.log(
					"✓ Validation error handled, status:",
					(error as ReverbApiError).status
				);
			}
		});
	});
